...
4. After (or during) training, run `plot_loss.py` to plot the loss and validation loss across epochs.

    - The script will also indicate the epoch where the minimum validation loss was achieved.
    
5. After training, run `model_valid.py` to test the model on the validation data.

    - Check that the reported `val_loss` is the same as indicated by `plot_loss.py`.

6. Run `model_test.py` to test the model on a given pulse.
...
7. Run `plot_frames.py` to plot the reconstructions generated by the model for the test pulse.

    - These will be built from the test data in `test_data.hdf`.
    
    - If needed, adjust `vmax` to change the dynamic range of the plots.

8. Run `create_movie.py` to produce a movie of the reconstructions for the test pulse.
